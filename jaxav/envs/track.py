...
from jaxav.utils.math import euler_to_quat, lerp, quat_rotate, uniform, sign, normalize, heading
...
def circle(t: ArrayLike, c: ArrayLike):
    sin_t = jnp.sin(t)
    cos_t = jnp.cos(t)
    x = jnp.stack([cos_t, sin_t, jnp.zeros_like(t)], axis=-1)
    return x
...
class Track(EnvBase):
    ...
    def reset(self, params: EnvParams, key: jax.random.KeyArray):
        ...
        pos_0 = quat_rotate(self.traj_func(self.T0, traj_c), traj_q) * traj_a
        pos_1 = quat_rotate(self.traj_func(self.T0 + 0.01*traj_w, traj_c), traj_q) * traj_a
        h = ( - pos_0)[:2]
        rot = euler_to_quat(jnp.array([0., 0., jnp.arctan2(h[1], h[0])]))
    ...
    def step(self, env_state: EnvState, action):
        ...
        # )
        # reward_heading = jnp.square(0.5*(reward_heading + 1.))
        reward_heading = jnp.exp(- 1.6 * jnp.linalg.norm(env_state.ref_heading[:2]-normalize(env_state.drone.heading[:2])[0]))
        reward = (reward_tracking + reward_tracking * reward_heading)[None]
        done = (
    ...
    def _obs(self, env_state: EnvState):
        t = (env_state.step + jnp.arange(5)*5) * env_state.dt * env_state.traj_w
        x = jax.vmap(self.traj_func, in_axes=(0, None))(
        ...
        ref_heading = normalize(-env_state.drone.pos[:2])[0]
        xy_heading = normalize(heading(env_state.drone.rot)[:2])[0]
        obs = jnp.concatenate([
        ...
        return obs, env_state.replace(
            ref_pos=x[0],
            ref_vel=(x[1]-x[0]) / env_state.dt/5,
    ...
    def render_matplotlib(self, states: Sequence[EnvState]):
        ...
        heading = np.stack([
            state.ref_pos, 
            state.ref_pos+jnp.zeros(3).at[:2].set(state.ref_heading)
        ], axis=-2)
        ref_heading_line = ax.plot(*heading.T, "--")[0]
        drone = Drone(ax, state.drone)
        drone_traj = Traj3D(ax, state.drone.pos)
        ref_traj = Traj3D(ax, state.ref_pos)
        time = ax.set_title("step 0")
        ...
        def update(state: EnvState):
            ...
            ref_heading_line.set_data(*heading.T[:2])
            ref_heading_line.set_3d_properties(heading.T[2])

            time.set_text(f"step {state.step}")
